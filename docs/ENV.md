...
| `PLAYWRIGHT_HEADED`            | `"true"` abre browser visible solo en verificación **individual** (`POST` sin `bulk=1`). Omitida o `false` → headless. `POST /api/monitor/bulk` siempre headless. Para **Altán `/consulta`** y **Diri** (captcha en Buscar), en práctica hace falta headed + `MONITOR_MANUAL_WAIT_MS` suficiente. | Opcional (default: `false`)  |
