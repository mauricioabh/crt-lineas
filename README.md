...
- **Observability:** `@sentry/nextjs` with PII scrubbing (CURP/credentials headers). Set `SENTRY_DSN` in `.env.local` after creating the `crt-lineas` project in Sentry. Dev probe: `GET /api/debug/sentry` (disabled when `VERCEL_ENV=production`); verify with `npm run test:observability`.
- **Async jobs:** Inngest fan-out for `POST /api/monitor/bulk` — each link runs in a durable worker with per-link retries; SSE polls `MonitorBulkJob` rows in Neon. Without `INNGEST_*`, bulk falls back to inline Playwright in the same request. Pattern reference: [portfolio `docs/inngest-pattern.md`](https://github.com/mauricioabh/portfolio/blob/master/docs/inngest-pattern.md).
